...
namespace _Prefab.Popup
{
    public abstract class popBase : MonoBehaviour
    {
        ...
        [SerializeField] private qtButton _btnClose;
        [SerializeField] protected TextMeshProUGUI _txtContent;
        ...
        {
            _btnClose?.onClick.AddListener(OnButtonCloseClick);
        }
        ...
        protected virtual void MoveOut()
        {
            switch (_moveOut)
            {
                ...
                case AnimType.ZoomIn:
                {
                    transform.DOScale(Vector3.zero, 0.1f).SetEase(Ease.Linear).OnComplete(() =>
                    {
                        gameObject.SetActive(false);
                    });
                    break;
